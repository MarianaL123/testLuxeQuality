...
describe('Login on webpage', () => {
    it('should login with invalid login', async () => {
        ...
        const isLoginXDisplayed = $(
          "#login_button_container > div > form > div:nth-child(1) > svg"
        );
        await expect(isLoginXDisplayed).toBeDisplayed();
        const isPasswordXDisplayed = $(
          "#login_button_container > div > form > div:nth-child(2) > svg"
        );
        ...
        async function getBorderColor(element) {
          const borderBottomColorProperty = await element.getCSSProperty(
            "border-bottom-color"
          );
        ...
        await expect(errorElem).toHaveText(
